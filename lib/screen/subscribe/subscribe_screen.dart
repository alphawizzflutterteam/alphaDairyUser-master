...
class _SubScribeScreenState extends State<SubScribeScreen> {
  ...
  String productSubscripitionDiscount = Get.arguments["product_subscripition_discount"] ??  Get.arguments["per"];
            ...
            children: [
              InkWell(
                onTap: () {
                  for (var element in cart.values) {
                    print(".........(CartLength)........" +
                        cart.values.length.toString());
                    print("^^^^^^^^^^(CartCheck)^^^^^^^^" +
                        element.cartCheck.toString());
                    if (element.storeID ==
                        storeDataContoller.storeDataInfo?.storeInfo.storeId) {
                                          ...
                                          if (subScibeController.deliveries != "") {
                                            if (subScibeController.selectDay != "") {
                                              if (subScibeController.selectTime != "") {
                                                ...
                                                item?.productSubscripitionDiscount = productSubscripitionDiscount;
                ...
                Expanded(
                  ...
                  GetBuilder<SubScibeController>(builder: (context) {
                    return GridView.builder(
                        ...
                        maxCrossAxisExtent:
                        80, // Maximum width or height of each item
                        ...
                        return InkWell(
                          onTap: () {
                            subScibeController.changeIndex(
                                index,
                                productDetailsController.dayinfo[index].remaining_days, productDetailsController
                                .dayinfo[index].title);
                            // cart = Hive.box<CartItem>('cart');
                            // var item = cart.get(attributeId);
                            // item?.selectDelivery =
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              border: Border.all(
                                ...
                                subScibeController.selectedMonth.contains(index)
                                    ? gradient.defoultColor
                                    : transparent,
                              ),
                              color: /*subScibeController.currentIndex == index*/subScibeController.selectedMonth.contains(index)
